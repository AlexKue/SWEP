...
echo "Installing traffic redirect to thin server (80 HTTP; sudo command required)"
sudo iptables -t nat -A PREROUTING -p tcp --dport 80 -j REDIRECT --to-port 3001
sudo iptables -t nat -I OUTPUT -p tcp -d 127.0.0.1 --dport 80 -j REDIRECT --to-ports 3001
echo "Installing traffic redirect to thin server (443 HTTPS; sudo command required)"
